...
contract DAIInterestRateModelV3 is JumpRateModelV2 {
     ...
     */
    constructor(uint blocksYear, uint jumpMultiplierPerYear, uint kink_, address pot_, address jug_, address owner_) JumpRateModelV2(blocksYear, 0, 0, jumpMultiplierPerYear, kink_, owner_) public {
     ...
     * @param kink_ The utilization point at which the jump multiplier is applied
     */
    function updateJumpRateModel(uint blocksYear, uint baseRatePerYear, uint gapPerYear, uint jumpMultiplierPerYear, uint kink_) external {
        require(msg.sender == owner, "only the owner may call this function.");
        blocksPerYear = blocksYear;
        gapPerBlock = gapPerYear / blocksPerYear;
        updateJumpRateModelInternal(blocksPerYear, 0, 0, jumpMultiplierPerYear, kink_);
        poke();
    }
